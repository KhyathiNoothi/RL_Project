...
class BellmanCartoonGame(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Bellman Equation + Dynamic Programming – Gridworld Value Iteration")
...
{
    "q": "During a Bellman sweep, cell A updates its value using its neighbour B's stored value. Then cell B is updated and its value rises. What is the correct description of what must happen next?",
    "options": [
        "A) Nothing — cell A's update is final for this sweep; it will re-evaluate B next sweep",
        "B) Cell A immediately re-updates itself because the Bellman operator is synchronous",
        "C) The sweep restarts from the beginning to preserve consistency",
        "D) Cell B propagates its new value to A within the same sweep by backtracking"
    ],
    "answer": "A) Nothing — cell A's update is final for this sweep; it will re-evaluate B next sweep"
},

{
    "q": "If γ is increased from 0.5 to 0.99, what happens to how much the agent values distant future rewards?",
    "options": [
        "A) They start to matter more in the value calculation",
        "B) They become completely irrelevant",
        "C) Nothing changes",
        "D) The reward signs reverse"
    ],
    "answer": "A) They start to matter more in the value calculation"
},

{
    "q": "Why is a trap state given a strongly negative reward instead of zero?",
    "options": [
        "A) So the value function naturally steers the agent's policy away from it",
        "B) To increase the grid size",
        "C) To deliberately slow down convergence",
        "D) Trap rewards don't affect any cell's value"
    ],
    "answer": "A) So the value function naturally steers the agent's policy away from it"
},

{
    "q": "After only one random cell update, why are most cells still at 0.00 instead of already reflecting the goal's reward?",
    "options": [
        "A) Value information has to propagate cell-by-cell over many separate updates",
        "B) The simulation has a bug",
        "C) Only the goal cell ever needs updating",
        "D) Rewards are reassigned randomly each run"
    ],
    "answer": "A) Value information has to propagate cell-by-cell over many separate updates"
},

    {
    "q": "The app tracks delta = |new V − old V| after every update. What does it mean when delta keeps shrinking toward 0 across many steps?",
    "options": [
        "A) The value function is settling into a stable solution",
        "B) The grid is being reset",
        "C) The agent has physically reached the goal",
        "D) The rewards are increasing over time"
    ],
    "answer": "A) The value function is settling into a stable solution"
},
{
...
{
    "q": "This app picks a random cell to update on each 'Next Bellman Step' click instead of sweeping cells in a fixed order. Will the grid still converge to the same final values no matter which cells happen to get picked first?",
    "options": [
...
"options":["A) To encourage the agent to avoid them",
           "B) To increase score",
...
"answer":"A"
    ...
    "options":["A) Values need time to propagate through the grid",
        ...
        "options":["A) Goal information propagates outward over sweeps",
           "B) Distant cells cannot be updated",
    ...
    "answer":"A"
        ...
        "options":["A) To allow values to converge gradually",
           "B) To reset values",
           "C) To randomize actions",
...
{
"q":"Cell A updates using B's old value. Later B's value increases. What happens next?",
"options":["A) A waits until the next sweep to benefit",
           "B) A immediately updates again",
           "C) The sweep restarts",
           "D) B forces A to update"],
"answer":"A"
},

{
"q":"What would make an algorithm model-free instead of model-based?",
"options":["A) Using a neural network",
           "B) Learning directly from experience without knowing transition probabilities",
           "C) Increasing γ",
           "D) Updating only visited states"],
"answer":"B"
},

{
"q":"What may happen if every step gives a positive reward instead of a penalty?",
"options":["A) The agent may wander forever",
           "B) The goal becomes unreachable",
           "C) Value iteration stops working",
           "D) Rewards disappear"],
"answer":"A"
}
    ...
    def create_ui(self):
    # Title
        tk.Label(self, text="Bellman Equation + Dynamic Programming – Gridworld Value Iteration",
...
# Add this 👇 to fix width cutting off
        ...
        # Quiz
        self.quiz_frame = tk.LabelFrame(right, text="Quick DP / Bellman Quiz", font=("Arial", 13, "bold"),
                                        bg="white", fg="#6f42c1")
...
# Reset feedback
    ...
    # Title
        ...
        posttest_questions = self.pretest_questions
        ...
        for q in posttest_questions:
            ...
            tk.Label(frame,text=q["q"],wraplength=700,justify="left",font=("Arial", 11, "bold"), bg="white").pack(anchor="w")

            var = tk.StringVar()

            self.posttest_vars.append(var)
    ...
    def export_all_data(self, duration, window):
        ...
        for i, var in enumerate(self.sus_vars):
            ans = var.get()
            if i % 2 == 0:   # positive questions
                sus_score += (ans - 1)
            else:            # negative questions
                sus_score += (5 - ans)
        sus_score = sus_score * 2.5

        filename = f"bellman_evaluation_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"

        with open(filename, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(["=== SESSION SUMMARY ==="])
            writer.writerow(["Start Time", datetime.now().strftime("%Y-%m-%d %H:%M:%S")])
            ...
            writer.writerow(["MCQ Accuracy (%)", round(self.mcq_correct / self.mcq_attempts * 100, 1) if self.mcq_attempts > 0 else 0])
            writer.writerow(["SUS Usability Score", round(sus_score, 1)])
            writer.writerow([])

            writer.writerow(["=== EVENT LOG ==="])
            writer.writerow(["Timestamp(s)", "Event", "Details", "Bellman_Steps", "MCQ_Correct", "MCQ_Total"])
        ...
        messagebox.showinfo("Export Successful", 
                            f"Evaluation data has been exported successfully!\n\n"
                            f"File: {filename}\n"
                            f"SUS Score: {round(sus_score, 1)} / 100")
        window.destroy()
        self.destroy()
    def open_feedback(self):
        feedback_win = tk.Toplevel(self)
        feedback_win.title("Feedback")
        feedback_win.geometry("400x350")
        feedback_win.configure(bg="white")

        tk.Label(feedback_win, text="💬 Your Feedback",
             font=("Arial", 16, "bold"), bg="white", fg="#0066cc").pack(pady=10)

        tk.Label(feedback_win, text="Rate this app:",
             font=("Arial", 11), bg="white").pack()

        rating_var = tk.IntVar(value=5)
        rating_frame = tk.Frame(feedback_win, bg="white")
        ...
        tk.Label(feedback_win, text="Comments:",
             font=("Arial", 11), bg="white").pack()

        text_box = tk.Text(feedback_win, height=6, width=40,
                       font=("Arial", 10), relief="ridge", bd=2)
        text_box.pack(pady=8, padx=10)
        ...
        tk.Button(feedback_win, text="Submit ✅",
              font=("Arial", 12, "bold"),
